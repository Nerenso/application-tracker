<template>
  <ContentBox>
    <section class="flex flex-col gap-4 p-4 md:p-6">
      ...
      <div
        ...
        v-html="workExperience.job_description"
      ></div>
    </section>
  </ContentBox>
  <BaseModal
  ...
  >
        ...
        <div>
          <BaseLabel label="Job Description" is-required>
            <p class="text-sm text-gray-600 dark:text-gray-400">
              Describe the responsibilities you had in this position, make sure
              to use active langauge.
            </p>
            <XTextarea
    ...
    <template #action>
      <XButton @click="submit" color="primary" :loading="loading"
        >Save Changes</XButton
      >
    </template>
  </BaseModal>
</template>
...
const submit = () => {
  ...
  workExperienceForm.patch(
    route("work-experience.update", props.workExperience.id),
    {
      onStart: () => {
        setLoading();
      },
