    ...
    <section class="detail-page w-full px-2 py-8 pt-6 flex flex-col gap-6">
      ...
      <div class="max-w-5xl mx-auto w-full bg-white p-4 md:p-6 border rounded-lg">
        ...
        <div class="flex items-center justify-between">
          ...
          <p v-else class="text-slate-500">No Tags Added</p>
          <button @click="showTagDropDown = !showTagDropDown" class="w-10 pl-[21px] pt-1.5 h-6">
        ...
        <div class="relative" :class="showTagDropDown ? 'extended' : 'shrunk'">
          <div class="w-full max-w-5xl overflow-hidden" :class="showTagDropDown ? '' : ''">
            ...
            <div class="mt-6 flex gap-2 justify-end">
              <XButton size="sm" @click="(showTagDropDown = false), syncTagsWithDBValues()">Cancel</XButton>
              <XButton size="sm" color="primary" @click="syncTags()">Save</XButton>
            </div>
          </div>
        </div>
      ...
      <ListingInfoBlock :listing="listing" />
    </section>
  </DashboardLayout>
</template>
...
import ListingInfoBlock from "@/Components/JobListing/ListingInfoBlock.vue";
